...
const DEFAULT_CATEGORIES = ["Tecnologia", "Decoracao", "Casa Moderna", "Virais", "Piscina", "Automacao", "Outros"];
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (savedProds) {
      try {
        ...
        setProducts(Array.isArray(parsed) && parsed.length > 0 ? parsed : defaultProducts);
      } catch { setProducts(defaultProducts); }
    } else {
      setProducts(defaultProducts);
    }

    const savedCats = localStorage.getItem("bella_categories");
    if (savedCats) {
      try {
  ...
  function saveCategories(cats: string[]) {
    localStorage.setItem("bella_categories", JSON.stringify(cats));
    setCategories(cats);
    flash();
  }
  ...
  function handleAdd() {
    ...
    saveProducts([...products, { ...form, id: Date.now().toString() }]);
  ...
  function startEditCat(index: number) {
  ...
  function saveEditCat() {
    ...
    if (!name) return;
    const updated = categories.map((c, i) => i === editCatIndex ? name : c);
    // Atualiza produtos que tinham a categoria antiga
    const oldName = categories[editCatIndex!];
    const updatedProds = products.map(p => p.category === oldName ? { ...p, category: name } : p);
    localStorage.setItem("bella_products", JSON.stringify(updatedProds));
  ...
  function deleteCategory(index: number) {
    if (!confirm(`Remover categoria "${categories[index]}"? Os produtos desta categoria ficarão sem categoria.`)) return;
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-4">
        <div className="w-full max-w-sm bg-white/5 border border-white/10 rounded-3xl p-8">
          ...
          <button onClick={handleLogin} className="w-full mt-4 bg-orange-500 hover:bg-orange-400 transition text-black font-bold py-3 rounded-xl">
            Entrar
          </button>
      ...
      <header className="border-b border-white/10 px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <a href="/" className="text-xs text-zinc-500 hover:text-zinc-300 transition border border-white/10 px-4 py-2 rounded-full">Ver Loja →</a>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="flex gap-2 mb-8">
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition border ${activeTab === "produtos" ? "bg-orange-500 text-black border-orange-500" : "bg-white/5 text-zinc-400 border-white/10 hover:text-white"}`}
          >
            📦 Produtos
          </button>
          <button
            onClick={() => setActiveTab("categorias")}
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition border ${activeTab === "categorias" ? "bg-orange-500 text-black border-orange-500" : "bg-white/5 text-zinc-400 border-white/10 hover:text-white"}`}
          >
            🏷️ Categorias
          </button>
          <button
            onClick={() => setActiveTab("stories")}
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition border ${activeTab === "stories" ? "bg-orange-500 text-black border-orange-500" : "bg-white/5 text-zinc-400 border-white/10 hover:text-white"}`}
          >
            📱 Stories
          </button>
        </div>
        ...
        {activeTab === "produtos" && (
          <>
              ...
              <div className="bg-white/5 border border-orange-500/30 rounded-2xl p-6 mb-8">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input value={form.image} onChange={e => setForm({ ...form, image: e.target.value })} placeholder="Cole o link da imagem (URL)..." className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-zinc-600 focus:outline-none focus:border-orange-500 text-sm" />
                ...
                {(form.image || form.title) && (
                  <div className="mt-6 p-4 bg-black/30 rounded-xl border border-white/5">
                    ...
                    <div className="flex gap-4 items-start">
                      {form.image && <img src={form.image} alt="preview" className="w-20 h-20 object-cover rounded-xl" />}
                      <div>
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={editId ? handleUpdate : handleAdd} className="bg-orange-500 hover:bg-orange-400 transition text-black font-bold px-8 py-3 rounded-xl text-sm">
                    {editId ? "Salvar Alteracoes" : "Adicionar Produto"}
                  </button>
                  <button onClick={handleCancel} className="bg-white/5 hover:bg-white/10 transition text-zinc-400 font-medium px-6 py-3 rounded-xl text-sm">
                    Cancelar
                  </button>
                </div>
              </div>
            ...
            <div className="space-y-3">
              <h2 className="font-bold text-zinc-400 text-sm uppercase tracking-wider mb-4">Produtos cadastrados ({products.length})</h2>
              {products.map(product => (
                <div key={product.id} className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-2xl p-4 hover:border-white/20 transition">
                  <img src={product.image || "https://via.placeholder.com/80"} alt={product.title} className="w-16 h-16 object-cover rounded-xl flex-shrink-0 bg-white/5" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="font-bold text-sm truncate">{product.title}</p>
                      {product.badge && <span className="bg-orange-500/20 text-orange-400 text-xs px-2 py-0.5 rounded-full flex-shrink-0">{product.badge}</span>}
                    </div>
        ...
        {activeTab === "categorias" && (
          <div>
            <div className="bg-orange-500/5 border border-orange-500/20 rounded-2xl p-5 mb-8 text-sm text-zinc-400">
              ...
              <p>As categorias aparecem como abas na loja e na Bella IA. Você pode criar, renomear, reordenar e remover. A ordem aqui é a ordem que aparece na loja.</p>
            </div>
            ...
            <div className="flex gap-3 mb-8">
              ...
              />
              <button onClick={addCategory} className="bg-orange-500 hover:bg-orange-400 transition text-black font-bold px-6 py-3 rounded-xl text-sm whitespace-nowrap">
                + Adicionar
              </button>
            </div>
            ...
            {/* LISTA DE CATEGORIAS */}
            <div className="space-y-3">
              <h2 className="font-bold text-zinc-400 text-sm uppercase tracking-wider mb-4">Categorias ({categories.length})</h2>
              {categories.map((cat, index) => (
                <div key={cat} className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-2xl px-4 py-3 hover:border-white/20 transition">

                  {/* ORDEM */}
                  <div className="flex flex-col gap-1">
                    <button onClick={() => moveCat(index, -1)} disabled={index === 0} className="text-zinc-600 hover:text-white disabled:opacity-20 text-xs leading-none">▲</button>
                    <button onClick={() => moveCat(index, 1)} disabled={index === categories.length - 1} className="text-zinc-600 hover:text-white disabled:opacity-20 text-xs leading-none">▼</button>
                  </div>
                  ...
                  <div className="flex gap-2">
                    {editCatIndex === index ? (
                      <>
                        <button onClick={saveEditCat} className="text-xs bg-orange-500 hover:bg-orange-400 text-black font-bold px-3 py-2 rounded-lg transition">Salvar</button>
                        <button onClick={() => setEditCatIndex(null)} className="text-xs bg-white/5 text-zinc-400 px-3 py-2 rounded-lg transition">Cancelar</button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => startEditCat(index)} className="text-xs bg-white/5 hover:bg-white/10 border border-white/10 text-zinc-400 hover:text-white px-3 py-2 rounded-lg transition">Renomear</button>
                        <button onClick={() => deleteCategory(index)} className="text-xs bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-red-400 px-3 py-2 rounded-lg transition">Remover</button>
                      </>
                    )}
                  </div>
    ...
    <div className="grid lg:grid-cols-2 gap-10">
      <div className="space-y-6">
          ...
          <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
            {products.map(p => (
              <button key={p.id} onClick={() => setSelected(p)}
                ...
                {selected?.id===p.id && <span className="text-orange-500 text-lg">✓</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        <button onClick={handleDownload} disabled={generating||!selected}
          ...
          {generating ? "Gerando..." : done ? "✓ Baixado! Agora é só postar 🚀" : "⬇ Baixar Card para Stories"}
        </button>
        {done && <div className="bg-green-500/10 border border-green-500/30 text-green-400 text-sm px-5 py-4 rounded-2xl">Card gerado! Tamanho 1080x1920px — pronto para Instagram e TikTok.</div>}
      </div>
      <div className="flex flex-col items-center">
        ...
        <p className="text-zinc-600 text-xs mt-4 text-center">Preview em tempo real • Download 1080x1920px</p>
      </div>
